package com.syndrome.sanguocard.mainfram.esroom.lobbyroom.privatemessage
{
	...
	*
	*/
	public class InviteSponse
	{
		public var invitedAccount:String;
		...
		public function InviteSponse(_lpmh:LobbyPrivateMessageHandler, _invitedAccount:String, _roomName:String, _zoneName:String)
		{
			lpmh = _lpmh;
			invitedAccount = _invitedAccount;
			roomName = _roomName;
			zoneName = _zoneName;
		}
		...
		public function ask():void
		{
			...
			pmr.esObject.setString(ACTION, ACTION_ASK_FOR_FIGHT);
			pmr.esObject.setString(INVITER_NICKNAME, meAccount.userVariableByName(Constant.GetUserInfo).value.getString("surname")+meAccount.userVariableByName(Constant.GetUserInfo).value.getString("name"));
			pmr.esObject.setString(INVITERS_ROOM, roomName);
			pmr.esObject.setString(INVITERS_ZONE, zoneName);
			pmr.userNames = new Array(invitedAccount);
			lpmh.sendPrivateMessage(pmr);
			AlertPanel.creatInstance().show("约战消息已经发送", null, null);
		}
		...
		public function cancel():void
		{
			...
			var meAccount:User = GameCom.getInstance().esServer.managerHelper.userManager.me;
			pmr.esObject = new EsObject();
			pmr.esObject.setString(ACTION, ACTION_CANCEL_FOR_FIGHT);
			pmr.esObject.setString(INVITER_NICKNAME, meAccount.userVariableByName(Constant.GetUserInfo).value.getString("surname")+meAccount.userVariableByName(Constant.GetUserInfo).value.getString("name"));
			pmr.esObject.setString(INVITERS_ROOM, roomName);
			pmr.esObject.setString(INVITERS_ZONE, zoneName);
			pmr.userNames = new Array(invitedAccount);
